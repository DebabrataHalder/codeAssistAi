...
class CodeEditor:
    def __init__(self, root):
        ...
        # Bind Ctrl+R to open a new PowerShell window
    ...
    def open_powershell(self, event=None):
        """Open a new PowerShell window at the location of the saved file."""
        if self.file_path:
            directory = os.path.dirname(self.file_path)
            subprocess.Popen(['powershell', '-NoExit', '-Command', f'Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd \'{directory}\'"'])
        else:
            messagebox.showwarning("Warning", "No file opened to determine the directory.")
